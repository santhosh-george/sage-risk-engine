CREATE OR REPLACE PROCEDURE genericmodelv3.public.SAGE_MEP_STEP_6 (iMODEL_YR IN INTEGER, iVRSN_NUM IN VARCHAR(10), iRUN_TYPE IN VARCHAR(20)) AS $$
DECLARE 
    sqlstr1 VARCHAR(max);
    sqlstr2 VARCHAR(max);
    sqlstr VARCHAR(max);
    ...
    C0 CURSOR FOR SELECT VARIABLE, CALCULATION FROM genericmodelv3.public.ME_INT_VARIABLE_CALC 
    ...
    SELECT UPPER(VARIABLE) AS VARIABLE FROM (
        ...
        ORDER BY VARIABLE;
BEGIN    
    TRUNCATE TABLE genericmodelv3.public.SAGE_OFILE_VARIABLE;
    sqlstr1 := 'INSERT INTO genericmodelv3.public.SAGE_OFILE_VARIABLE (MODEL_YR, VRSN_NUM, RUN_TYPE ';
    ...
    sqlstr2 := concat(sqlstr2,' FROM genericmodelv3.public.SAGE_OFILE_5 ');
    sqlstr := concat(sqlstr1, concat(' ', sqlstr2));

	EXECUTE sqlstr;
    DROP TABLE genericmodelv3.public.SAGE_OFILE_5;
END;
$$ LANGUAGE plpgsql;